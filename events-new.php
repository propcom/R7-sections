...
<?
    ...
    if(isset($_GET['month'])){
        $current_month = date('m', strtotime($_GET['month']));
    } else {
        $current_month = date('m');
    }

    $events_first = $calendar->get_events('2016-'.$current_month.'-01', '2016-'.$current_month.'-14');
    $events_second = $calendar->get_events('2016-'.$current_month.'-15', 'last day of this month');

    $events = array_merge($events_first, $events_second);
?>
        ...
        <div class="events__container" id="events-container">
            <div class="events__months events__months--bold">
                <ul>
                    ...
                    <li>
                        <a href="/<?= \Arr::get($config, 'url', 'events');?>?month=<?= $month;?>#events-container"
                        <? if(isset($_GET['month']) && date('m', strtotime($_GET['month'])) == date('m', strtotime($month))){ echo 'class="active";'; } ?>
                        <? if(!isset($_GET['month']) && date('m') == date('m', strtotime($month))){ echo 'class="active";'; } ?>>
                            <?= $month;?>&rarr;
                        </a></li>
                    <? endfor; ?>
                </ul>
            ...
            <? if($events) :?>
                <? foreach($events as $date => $date_events):?>
                    <? foreach($date_events as $event):
                        ...
                        <div class="events__entry events__entry--narrow <?= $pastDate; ?>" id="<?= $event->id;?>"  <? if($pastDate != '' && !isset($_GET['month'])){ echo 'style="display: none";'; } ?>>
